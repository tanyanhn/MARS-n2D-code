...
bool checkIsBounded(const T &iterator, Real tol)
{
  ...
  std::sort(edges.begin(), edges.end(), [](const Segment<2> &lhs, const Segment<2> &rhs) {
    Real k1 = lhs.slope();
    Real k2 = rhs.slope();
...
template <>
void SegmentedRealizableSpadjor<2>::applySegmentation(const vector<Curve<Dim, 2>> &aSpadjor, Real tolForSegmentation)
{
  std::vector<int> idxOfCurve;
  std::vector<int> idxOfPiece;
  std::vector<Segment<2>> segs = collapseToSeg(aSpadjor, idxOfCurve, idxOfPiece);
  ...
  std::vector<std::vector<Real>> brks(aSpadjor.size()); // [which curve][which piece]
  for(const auto &info : itsInfo) {
    ...
    if(incident.size() > 2) {
      ...
      if(incident[0] != incident[1])
        assert(!"Invalid input of Yin set : may contain proper intersections. ");
    }
  }
  // split the curves
...
meet(const SegmentedRealizableSpadjor<2> &lhs, const SegmentedRealizableSpadjor<2> &rhs, Real tol)
{
  const int Dim = 2;
  const int Order = 2;
  using std::pair;
  ...
  const std::vector<Curve<Dim, Order>> *bdriesOfOperands[] = { &(lhs.segmentedCurves), &(rhs.segmentedCurves) };
  ...
  vector<pair<Vec<Real, Dim>, vector<int>>> itsInfo;
  {
    ...
    for(auto &p : rawInfo) {
      ...
      if(incident.size() > 2 || (incident[0]<numSeg[0]) != (incident[1]<numSeg[0]))
  ...
  for(const auto &i : itsInfo) {
    for(int j : i.second) {
      // for every Jordan curve in the realizable spadjor, find out all the breaks
...
template <>
SegmentedRealizableSpadjor<2>::SegmentedRealizableSpadjor(const vector<Curve<Dim, 2>> &aSpadjor, Real tolForSegmentation)
{
...
template <>
SegmentedRealizableSpadjor<4>::SegmentedRealizableSpadjor(const vector<Curve<Dim, 4> > &aSpadjor, Real tolForSegmentation)
{
  // *******************************************************
...
template <>
SegmentedRealizableSpadjor<2>::SegmentedRealizableSpadjor(std::istream &is, Real tolForSegmentation)
{
...
template <>
SegmentedRealizableSpadjor<4>::SegmentedRealizableSpadjor(std::istream &is, Real tolForSegmentation)
{
