...
void PastingMap<Order, Selector>::
    removeEdge(const rVec& oldtail, vector<Crv> &repo, typename vector<Crv>::const_iterator eit)
{
...
void PastingMap<Order, Selector>::formClosedLoops(vector<Crv>& outCont)
{
  ...
  while(!graph.empty()) {
    ...
    if(h != footprint.cend()) { // a cycle is formed
      outCont.push_back(jordan.extract(h->second, jordan.domain().hi()[0], tol));
