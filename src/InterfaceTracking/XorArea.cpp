...
tuple<bool, Real, Real> segIntSeg(Point lp1, Point hp1, Point lp2, Point hp2, Real tol)
{
...
Real segIntCurve(Point pt, Point grad, const Crv &crv, Real tol)
{
    ...
    for (int i = 0; i < Num; i++)
    {
        tuple<bool, Real, Real> ifint = segIntSeg(pt - grad, pt + grad, pts[i], pts[i + 1], tol);
        if (std::get<0>(ifint) == true)
            intersect.push_back(make_tuple(i, std::get<1>(ifint), std::get<2>(ifint) * 0.5 + 0.5));
    }
    if(intersect.size() == 0)
    {
