...
class MARS2DIMV : public MARS<2, Order, VelocityField> {
 ...
 private:
  ...
  Vector<unsigned int> splitLongEdges(
      const VelocityField<2> &v, Vector<Point> &pts, const Vector<Real> &knots,
...
class MARS2DIMV<Order, VectorFunction> : public MARS<2, Order, VectorFunction> {
 ...
 public:
  ...
  MARS2DIMV(TimeIntegrator<2, VectorFunction> *_TI, Real hL, Real rtiny = 0.1)
      : Base(_TI), chdLenRange(Vec<Real, 1>(rtiny * hL), Vec<Real, 1>(hL)) {}
...
class MARS2DIMV<Order, VectorOnHypersurface>
 ...
 public:
  ...
  MARS2DIMV(TimeIntegrator<2, VectorOnHypersurface> *_TI, Real hL,
            Real rtiny = 0.1)
      : Base(_TI), chdLenRange(Vec<Real, 1>(rtiny * hL), Vec<Real, 1>(hL)) {}
