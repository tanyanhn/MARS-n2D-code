...
Vec<Real,Dim> Segment<Dim>::substitute(const rVec &query, Real tol) const
{
  ...
  Real ratio = (query[Dim-1]-p[0][Dim-1]) / (p[1][Dim-1]-p[0][Dim-1]);
  rVec r = p[0] + (p[1] - p[0]) * ratio;
  return r;
}

template <> inline
bool Segment<2>::parallel(const Segment<2> &that, Real tol) const
{
  const rVec &p3 = that.p[0];
  const rVec &p4 = that.p[1];
  rVec A[2];
  A[0] = p[1]-p[0]; A[1] = p3-p4;
  return std::abs(cross(A[0], A[1])) / norm(A[0]) <= tol;
}

template <> inline
Real Segment<2>::area(const rVec &q) const
{
...
template <int Dim> inline
typename Segment<Dim>::intsType solveForOverlie(Vec<Real,Dim> &p1, Vec<Real,Dim> &p2,
                                                Vec<Real,Dim> &p3, Vec<Real,Dim> &p4,
                                                Vec<Real,Dim> &pOut1, Vec<Real,Dim> &pOut2,
...
inline
Segment<2>::intsType intersect(const Segment<2> &thisSeg, const Segment<2> &thatSeg,
                               Vec<Real,2> &pOut1, Vec<Real,2> &pOut2, Real tol)
{
  using rVec = Segment<2>::rVec;
  rVec p1 = thisSeg.p[0];  rVec p2 = thisSeg.p[1];
  rVec p3 = thatSeg.p[0];  rVec p4 = thatSeg.p[1];

  rVec A[2], b;
  A[0] = p2-p1; A[1] = p3-p4; b = p3-p1;
...
template <int Dim> inline
bool isInInterval(const Vec<Real,Dim> &q, const Vec<Real,Dim> &p1, const Vec<Real,Dim> &p2,
                  Real tol, int majorDim)
{
  if(q[majorDim] >= std::min(p1[majorDim],p2[majorDim])-tol
...
template <int Dim> inline
bool Segment<Dim>::contain(const rVec &q, Real tol) const
{
  if(!parallel(Segment<Dim>(q,p[0]), tol))
    return false;
  auto delta = p[1] - p[0];
