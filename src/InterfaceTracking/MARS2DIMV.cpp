...
void MARS2DIMV<Order, VectorFunction>::discreteFlowMap(
    const VectorFunction<2> &v, Vector<Point> &pts, Real tn, Real dt) {
...
Vector<unsigned int> MARS2DIMV<Order, VectorFunction>::removeSmallEdges(
  ...
  while (true) {
    ...
    */
    ...
    if (!removeIMV(ids, pts, (chdLenRange.lo())[0])) break;
  }
...
Vector<unsigned int> MARS2DIMV<Order, VectorFunction>::splitLongEdges(
    const VectorFunction<2> &v, Vector<Point> &pts, const Vector<Real> &knots,
    const Vector<Polynomial<Order, Point>> &polys, Real tn, Real dt) {
  ...
  auto addpt = [&](int &count, Point pt) {
    res[count] = pt;
    count++;
    if (count >= (int)res.size()) res.resize(2 * count);
    return;
  };
...
void MARS2DIMV<Order, VectorFunction>::timeStep(const VectorFunction<2> &v,
  ...
  for (int id = 0; id < N; id++) {
    ...
    if (vbrk[id].empty()) {
      ...
      auto localtimestep = [&](const int pre, const int pos,
                               unsigned int &back) {
        ...
        if (!respts.empty()) respts.pop_back();
...
template <int Order>
bool splitIMV(Vector<bool> &ids, Vector<Point> &oldpts,
              const Vector<Real> &knots,
              Vector<Polynomial<Order, Point>> &polys, const Vector<Real> &dist,
              Real highBound) {
  int num = oldpts.size();
  Vector<Point> res(2 * num);
...
Vector<unsigned int> MARS2DIMV<Order, VectorOnHypersurface>::splitLongEdges(
    const VectorOnHypersurface<2> &v, Vector<Point> &pts,
    const Vector<Real> &knots, Vector<Polynomial<Order, Point>> &polys, Real tn,
    Real dt) {
  int num = pts.size();
