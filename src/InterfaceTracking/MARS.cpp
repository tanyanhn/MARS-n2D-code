...
void MARS<Dim, Order>::trackInterface(const IT_VectorFunction<Dim> &v, YS &ys, Real StartTime, Real dt, Real EndTime, bool output, std::string fName, int opstride)
{
    ...
    if (output == true)
    {
        std::ofstream of(std::string(fName + "_Start" + ".dat"), std::ios_base::binary);
        ys.dump(of);
    }
    while (T < EndTime)
    {
        ...
        {
            std::ofstream of(std::string(fName + "_End" + ".dat"), std::ios_base::binary);
            ys.dump(of);
            break;
        }
        if (output == true && step % opstride == 0)
        {
            ...
            tmps << step;
            std::ofstream of(std::string(fName + "_Step" + tmps.str() + ".dat"), std::ios_base::binary);
            ys.dump(of);
        }
        step++;
    }
