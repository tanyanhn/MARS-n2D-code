...
void YinSet<2, Order>::buildHasse(Real tol) {
  ...
  for (int i = 0; i < numCurves; ++i) {
    ...
    auto loc =
        locater(collapseToSeg(segmentedCurves[i]), queries, boundedness[i]);
...
YinSet<2, 2> intersect(const YinSet<2, 2>& lhs,
                       ...
                       Real tol) {
  // First calculate the boundary curves of the intersection. No segmentation
  // map is needed.
...
void YinSet<2, Order>::reFitCurve(unsigned int i) {
  ...
  while (start != end) {
    sims.insert(Simplex{std::initializer_list<unsigned int>{start->second}});
    start++;
  }
