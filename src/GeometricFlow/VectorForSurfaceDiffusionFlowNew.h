...
template <int Order>
class VectorForSurfaceDiffusionFlowNew<2, Order>
    : public VectorOnHypersurface<2> {
 private:
  template <class T>
  using Vector = std::vector<T>;
...
template <int Order>
const Vector<Point> VectorForSurfaceDiffusionFlowNew<2, Order>::operator()(
  ...
  Vector<Real> tmp(num);
  for (int i = 0; i < num; i++) {
    tmp[i] = -der4[i][0] * der1[i][1] - 2 * der3[i][0] * der2[i][1] -
             der2[i][0] * der3[i][1] + der4[i][1] * der1[i][0] +
             2 * der3[i][1] * der2[i][0] + der2[i][1] * der3[i][0];
  }
...
template <int Order>
const Tensor<Real, 2> VectorForSurfaceDiffusionFlowNew<2, Order>::getJacobi(
    const Vector<Point>& pts, Real t) const {
