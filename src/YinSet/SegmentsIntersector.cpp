...
SegmentsIntersector::StatusType::iterator SegmentsIntersector::addToStatus(
    cpSeg cit) {
  Bundle *v = new Bundle;
  v->push_back(cit);
  ...
  if (!ret.second) {  // an already existed bundle implies overlap
    ...
    for (auto i = bdl->cbegin(); i != bdl->cend(); i++) findNewEvents(cit, *i);
    bdl->push_back(cit);
  }
  // save the reference to the status structure, will be useful in
  // delFromStatus()
  backref[cit - lineSetBegin] = ret.first;
  return ret.first;
}

SegmentsIntersector::StatusType::iterator SegmentsIntersector::delFromStatus(
    cpSeg cit) {
  StatusType::iterator ret = backref[cit - lineSetBegin];
  StatusType::iterator r2r = ret;
  r2r++;
  Bundle *bdl = *ret;
...
auto SegmentsIntersector::findIntersections(const std::vector<Seg> &lineSet)
    -> ResultType {
  ...
  while (!event.empty()) {
    ...
    for (auto i = incident.cbegin(); i != incident.cend(); i++) {
      if (i->second != LOWER) {  // except L(p)
        ...
        if (leftmost == status.end() || bdl_cmp(*j, *leftmost)) leftmost = j;
        if (rightmost == status.end() || bdl_cmp(*rightmost, *j)) rightmost = j;
      }
    }
