...
template <int Order>
const Vector<Point> VectorForCurvatureFlow<2, Order>::operator()(
...
template <int Order>
const Tensor<Real, 2> VectorForCurvatureFlow<2, Order>::getJacobi(
    const Vector<Point>& pts, Real t) const {
  const int num = pts.size() - 1;
  const Curve<2, 4> crv = fitCurve<4>(pts, Curve<2, 4>::periodic);
  Tensor<Real, 2> res(Vec<int, 2>{num * 2, num * 2});
  for (int l = 0; l < num * 2; l++)
    for (int m = 0; m < num * 2; m++) res(l, m) = 0.0;
  for (int i = 0; i < num; i++) {
