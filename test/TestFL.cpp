...
void testFL() {
  ...
  */
  ...
  for (int j = 0; j < loop; j++) {
    ...
    for (int i = 1; i < n; i++) {
      pts.push_back({(center[0] + radio * cos(6 * 2 * M_PI / n * i)) *
                         cos(2 * M_PI / n * i),
                     (center[1] + radio * cos(6 * 2 * M_PI / n * i)) *
                         sin(2 * M_PI / n * i)});
    }
    pts.push_back({center[0] + radio, 0});
    // cout << "----------- n = " << n << ": ------------" << endl;
  ...
  // endl;
  //   cout << "1-norm: " <<
  //   OneNormVelocity(Extrapolation(list2[i],list2[loop-1]),list2[i],2*M_PI*radio/n)
  //   <<
  // endl;
  //   n*=2;
  // }
  ...
  for (int i = 1; i < n; i++) {
    rpts.push_back(
        {nradio * cos(2 * M_PI / n * i), nradio * sin(2 * M_PI / n * i)});
  }
  rpts.push_back({nradio, 0.0});
  auto rcrv = fitCurve<4>(rpts, Curve<2, 4>::periodic);
  ...
  // auto result2 = exactError(Vector<Crv>(it2, it3), rcrv, tol);
  cout << "Error and ratio:" << endl;
  for (int l = 0; l < (int)result1.size(); l++) cout << result1[l] << endl;
  for (int l = 0; l < (int)time1.size(); l++) cout << time1[l] << endl;
}
