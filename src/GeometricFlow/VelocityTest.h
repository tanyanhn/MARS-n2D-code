...
// public:
//   CircleInnerNormal(Point Center, Real Radio) : center(Center),
//   radio(Radio){};
...
Real OneNormVelocity(const Vector<Point>& v1, const Vector<Point>& v2,
  ...
  assert(num1 == num2);
  Vector<Real> diff;
  for (int i = 0; i < num1; i++) diff.push_back(norm(v1[i] - v2[i], 2));
...
Vector<Point> Extrapolation(const Vector<Point>& v1, const Vector<Point>& v2) {
  const int num1 = v1.size();
  const int num2 = v2.size();
  assert((num2 - 1) % (num1 - 1) == 0);
  const int r = (num2 - 1) / (num1 - 1);
  Vector<Point> ref;
  for (int i = 0; i < num1 - 1; i++) ref.push_back(v2[i * r]);
  ref.push_back(v2[0]);
  return ref;
}
...
Vector<Point> CircleCurvature(const Real radio, const int n) {
  ...
  for (int i = 1; i < n; i++) {
    pts.push_back({-1.0 / radio * cos(2 * M_PI / n * i),
                   -1.0 / radio * sin(2 * M_PI / n * i)});
  }
  pts.push_back({-1.0 / radio, 0.0});
  return pts;
}
...
Vector<Point> CircleCurvatureLaplacian(const Real radio, const int n) {
  Vector<Point> pts;
  for (int i = 0; i <= n; i++) pts.push_back({0.0, 0.0});
  return pts;
}
