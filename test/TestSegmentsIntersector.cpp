...
bool my_isspace(const char &c) { return isspace(c); }

vector<Segment<2>> TestSegmentsIntersector::readInputFile(
    const string &filename, Real &tol) {
  std::cout << "filename: \n" << filename;
  ...
  for (;;) {
    ...
    if (!input || all_of(buf.begin(), buf.end(), my_isspace)) break;
...
SegmentsIntersector::ResultType TestSegmentsIntersector::readAnswerFile(
...
bool TestSegmentsIntersector::verify(const SegmentsIntersector::ResultType &r1,
  ...
  for (auto j = r2.begin(); j != r2.end(); i++, j++) {
    if (pt_cmp.compare(i->first, j->first) != 0) return false;
